...
def __check_2D_shape(actual, expected):
    """
...
def __check_2D_vals(check_val_fn, actual, expected):
    """
    ...
    Returns
        - None if no problem is found, otherwise an error message
    """
    err_msg = __check_2D_shape(actual, expected)
    if err_msg is not None:
        return "\n\n" + err_msg

    err_msg = "Actual and expected values do not match at location ({}, {}).\n"
    err_msg += "   Expected value: {}\n"
    err_msg += "   Actual value: {}"

    for i, row in enumerate(expected):
        for j, val in enumerate(row):
            if not check_val_fn(val, actual[i][j]):
                return err_msg.format(i, j, val, actual[i][j])
    return None

def __check_2D_type(check_element_type_fn, actual, error_msg=None):
    """
...
def check_rgb_image(actual, expected):
    ...
    err_msg = __check_2D_vals(lambda a, e: a == e, actual, expected)
    if err_msg is not None:
        return "\n\n" + err_msg
