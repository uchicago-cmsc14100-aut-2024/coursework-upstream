...
def check_2D_vals(check_val_fn, actual, expected):
    ...
    Returns
        - None if no problem is found, otherwise an error message
    """
    err_msg = check_2D_shape(actual, expected)
...
def check_rgb_image(actual, expected):
    ...
    err_msg = check_2D_type(__check_rgb_color, actual, type_error_msg)
    if err_msg is not None:
        return "\n\n" + err_msg
    ...
    err_msg = check_2D_vals(lambda a, e: a == e, actual, expected)
    if err_msg is not None:
        return "\n\n" + err_msg
